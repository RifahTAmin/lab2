...
# No. of unique artists represented in the collection calculated
...
#Introduce the_tate and the_tate_artists dataset DataExplorer used#
...
artist_missing_year_count <- artist_missing_year_count |>
  mutate(cumulative_percentage = cumsum(percentage)) # The cumulative percentage of missing 'year' values are calculated # 
...
# Display and extract the top 10 artists with the highest works #
...
# Select only the columns for artist and title to counting distinct combinations #
distinct_artist_title_pair <- the_tate |> select(artist,title) |> distinct()

distinct_count <- nrow(distinct_artist_title_pair)

print(total_rows) # Printing the total number of rows in the dataset #

print(distinct_count)  # Printing distinct pairs only #
...
selected_artworks <- the_tate |>
  ...
  drop_na() # Removing rows that contain NA values#

ordered_artworks <- selected_artworks |>
  arrange(area_cmsquare)

# Removing largest artwork in the collection #
largest_artwork <- ordered_artworks |>
  slice_tail(n = 1)

# Removing smallest artwork in collection #
smallest_artwork <- ordered_artworks |>
  slice_head(n = 1)
# Printing both the smallest and largest artwork details #
...
```{r}
# Installing packages for reading the csv file
...
# Converting the "Year" column in summary_data to character
summary_data <- summary_data |>
  mutate(Year = as.character(Year))

# Combining the summary rows with the summary_data
